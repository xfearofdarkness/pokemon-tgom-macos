...
def get_raw_keystates
    if $win32KeyStates == nil
        if Input.respond_to?(:raw_key_states)
            ...
            $win32KeyStates = System.raw_key_states
        else
            $win32KeyStates = []
        end
    end
...
module Win32API
    module Kernel32
        ...
        GetTempPath = GetTempPathA = ->(buflen, buf) do
            ...
            tmp = tmp + "/" unless tmp.end_with?("/", "\\")
            # Write into buffer if provided
            if buf && buflen.to_i > 0
                s = tmp[0, buflen.to_i - 1]
                buf[0, s.bytesize] = s
                buf[s.bytesize] = "\0" if s.bytesize < buflen.to_i
                return s.bytesize
            end
            tmp.bytesize
        end
    ...
    module User32
        ...
        GetWindowThreadProcessId = ->(hwnd, pid_out) do
            if pid_out
                begin
                    pid_out[0, 4] = [1].pack("L")
                rescue
                end
            end
            1
        end
        RegisterHotKey = ->(*args) { 1 }
        MessageBox = MessageBoxA = ->(*args) { 1 }
        SetCapture = ->(*args) { 1 }
        ReleaseCapture = ->(*args) { 1 }
        PostMessage = PostMessageA = ->(*args) { 1 }
    end
    ...
    def self.new(dllname, func, *rest)
        dll = dllname.to_s
        if dll.downcase.end_with?(".dll")
            dllname = dll[0...-4]
        else
            dllname = dll
        end
        dllname = dllname.downcase
